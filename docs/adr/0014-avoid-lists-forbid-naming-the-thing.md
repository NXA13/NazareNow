...
this quantity everywhere it travels — the API schema, the store column, the frontend type, and
the Predictive Distribution that computes it. Only the floor on it and the predicate reading it
had borrowed "confidence". So the identifiers were derived from the name the quantity already
answered to:
...
it was already in the schema, and inventing a domain term for a quantity the whole pipeline
already agrees on would have added a third vocabulary rather than removing a second.
...
**Prose that named the floor moved with it; prose that names Model Spread did not.** Three
documents called this bar "the Go Call confidence floor" — `analysis/calibration/README.md`,
`analysis/forecast_error/README.md`, and one test docstring. That phrasing is legal under the
rule above, and it was still reworded, because it named *the thing that was renamed*. Leaving it
would be the two-names-across-a-boundary state ADR 0013 objected to, merely moved into prose.
The prose that names Model Spread — `spread.py`, `decision.py`'s module docstring, `api.ts` —
is untouched, because there the word is doing the job the glossary gives it.
...
`ast` and scans `frontend/src` for declarations, type and interface fields, class names and test
ids, failing if any identifier contains "confiden". Prose and comments are ignored, which is the
rule above made executable rather than a blanket ban.

It is scoped to the two shipped trees. `analysis/` is deliberately outside it, because that is
where the legal provenance sense lives. The frontend's tests sit inside `frontend/src` and are
swept with it, which is worth having: the old test id lived in `Forecast.test.tsx`. `backend/tests`
is not swept, because the guard file is itself named for confidence and would flag itself.
...
- `GO_CALL_CONFIDENCE` is gone from an interface other code imports; anything reading it by
  name fails loudly at import rather than silently.
